<template>
	<view :class="['container', isDark ? 'dark-mode' : '']">
		...
		<view v-if="showForm" class="card form-card">
			...
			<view class="payer-section">
				...
				<view class="payer-list">
					<view v-for="(m, idx) in members" :key="m._id"
						:class="['payer-tag', selectedPayerIds.includes(m._id) ? 'payer-active' : '']"
		...
		<view class="search-bar">
			<input class="search-input" v-model="keyword" :placeholder="t('fee.searchPh')" @input="onSearch" />
			<picker :range="filterPayerNames" @change="onFilterPayer" :value="filterPayerIndex">
				<view class="filter-btn">{{ filterPayerNames[filterPayerIndex] }} ▾</view>
			</picker>
		</view>

		<!-- 按月分组列表 -->
		<scroll-view scroll-y refresher-enabled :refresher-triggered="refreshing" @refresherrefresh="onRefresh" style="height: calc(100vh - 580rpx);">
			<view v-if="groupedList.length === 0" class="empty-tip">
				<text>{{ keyword || filterPayerIndex > 0 ? t('fee.noResult') : t('fee.noRecord') }}</text>
			</view>
			<view v-for="(group, gi) in groupedList" :key="gi" class="month-group">
				<view class="month-header">
					<text class="month-label">{{ group.month }}</text>
					<text class="month-sum">{{ t('fee.monthTotal') }} ¥{{ group.total }}</text>
				</view>
				<view v-for="(item, index) in group.items" :key="item._id || index" class="fee-item" @tap="showDetail(item)">
					<view class="fee-icon-wrap">
						<text class="fee-icon">💰</text>
					</view>
					<view class="fee-info">
						<text class="fee-title">{{ item.title }}</text>
						<text class="fee-sub">{{ item.creatorName }} · {{ formatDate(item.createdAt) }}</text>
					</view>
					<view class="fee-right">
						<text class="fee-amount">¥{{ item.amount }}</text>
						<text class="fee-avg">{{ t('fee.perPerson') }} ¥{{ item.perPerson }}</text>
					</view>
				</view>
			</view>
		</scroll-view>
		...
		<view v-if="detailVisible" class="modal-mask" @tap="detailVisible = false">
			<view class="modal-content" @tap.stop>
				...
				<view class="detail-row">
					...
					<text class="detail-value amount-highlight">¥{{ detailItem.amount }}</text>
				</view>
				<view class="detail-row">
				...
				<view class="detail-row">
					...
					<text class="detail-value">{{ formatFull(detailItem.createdAt) }}</text>
				</view>
			</view>
		</view>
	</view>
...
const filterPayerNames = computed(() => [t('fee.allPayer'), ...memberNames.value])
...
const onFilterPayer = (e) => { filterPayerIndex.value = Number(e.detail.value) }
...
const currentMonthList = computed(() => feeList.value.filter(i => formatMonth(i.createdAt) === currentMonth.value))
const monthSpend = computed(() => currentMonthList.value.reduce((s, i) => s + (i.amount || 0), 0).toFixed(2))
...
const submitFee = async () => {
	...
	try {
		...
		if (res.code === 200) {
			...
			form.title = ''; form.amount = ''; form.remark = ''; selectedPayerIds.value = []
...
.stats-card {
	...
	border-radius: 16rpx; padding: 32rpx 0; margin-bottom: 24rpx;
}
.dark-mode .stats-card { background: linear-gradient(135deg, #0d47a1, #1565c0); }
.stats-item { flex: 1; text-align: center; }
.stats-value { font-size: 40rpx; font-weight: bold; color: #fff; display: block; }
.stats-label { font-size: 24rpx; color: rgba(255,255,255,0.8); display: block; margin-top: 8rpx; }
.stats-divider { width: 1rpx; height: 60rpx; background: rgba(255,255,255,0.3); }

/* 搜索栏 */
.search-bar { display: flex; gap: 16rpx; margin: 24rpx 0 16rpx; align-items: center; }
.search-input {
	flex: 1; height: 68rpx; background: var(--bg-card); border: 1rpx solid var(--border-color);
	border-radius: 34rpx; padding: 0 28rpx; font-size: 26rpx; color: var(--text-primary);
}
.filter-btn {
	height: 68rpx; line-height: 68rpx; padding: 0 24rpx; background: var(--bg-card);
	border: 1rpx solid var(--border-color); border-radius: 34rpx;
	font-size: 24rpx; color: var(--color-primary); white-space: nowrap;
}
...
.payer-section { margin-bottom: 20rpx; }
.payer-label { font-size: 28rpx; color: var(--text-secondary); display: block; margin-bottom: 12rpx; }
.payer-list { display: flex; flex-wrap: wrap; gap: 12rpx; }
.payer-tag {
	padding: 12rpx 24rpx; border-radius: 32rpx; font-size: 26rpx;
	background: var(--bg-input); color: var(--text-secondary);
	border: 1rpx solid var(--border-color);
}
.payer-active {
	background: #e6f4ff; color: var(--color-primary);
	border-color: var(--color-primary);
}
.dark-mode .payer-active { background: #1a2a3a; }

/* 月份分组 */
.month-group { margin-bottom: 8rpx; }
.month-header {
	display: flex; justify-content: space-between; align-items: center;
	padding: 20rpx 8rpx 12rpx; border-bottom: 1rpx solid var(--border-light);
}
.month-label { font-size: 28rpx; font-weight: bold; color: var(--text-primary); }
.month-sum { font-size: 24rpx; color: var(--text-hint); }

/* 费用条目 */
.fee-item {
	display: flex; align-items: center; padding: 24rpx 16rpx;
	background: var(--bg-card); border-radius: 12rpx; margin-top: 12rpx;
}
.fee-item:active { background: var(--bg-hover, #f0f0f0); }
.fee-icon-wrap {
	width: 72rpx; height: 72rpx; border-radius: 50%;
	background: #FFF3E0; display: flex; align-items: center; justify-content: center;
	margin-right: 20rpx; flex-shrink: 0;
}
.dark-mode .fee-icon-wrap { background: #2a2000; }
.fee-icon { font-size: 36rpx; }
.fee-info { flex: 1; overflow: hidden; }
.fee-title { font-size: 28rpx; color: var(--text-primary); display: block; font-weight: 500; }
.fee-sub { font-size: 22rpx; color: var(--text-hint); display: block; margin-top: 6rpx; }
.fee-right { text-align: right; flex-shrink: 0; margin-left: 16rpx; }
.fee-amount { font-size: 30rpx; color: #e74c3c; font-weight: bold; display: block; }
.fee-avg { font-size: 20rpx; color: var(--text-hint); display: block; margin-top: 4rpx; }
...
.modal-content {
	width: 85%; background: var(--bg-card, #fff); border-radius: 20rpx;
	padding: 40rpx 32rpx; max-height: 80vh;
}
.modal-header {
	display: flex; justify-content: space-between; align-items: center;
	margin-bottom: 32rpx;
...
.detail-row {
	display: flex; justify-content: space-between; align-items: center;
	padding: 20rpx 0; border-bottom: 1rpx solid var(--border-light);
}
.detail-row:last-child { border-bottom: none; }
.detail-label { font-size: 28rpx; color: var(--text-secondary); }
.detail-value { font-size: 28rpx; color: var(--text-primary); font-weight: 500; }
.amount-highlight { color: #e74c3c; font-size: 34rpx; font-weight: bold; }
</style>
