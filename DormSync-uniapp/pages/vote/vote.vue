<template>
	<view :class="['container', isDark ? 'dark-mode' : '']">
		...
		<view v-if="showForm" class="card form-card">
			<view class="form-title">{{ editingId ? t('vote.editTitle') : t('vote.formTitle') }}</view>
			<input class="input" v-model="form.title" :placeholder="t('vote.topicPh')" />
			<view v-if="!editingId" v-for="(opt, idx) in form.options" :key="idx" class="option-row">
			...
			<view class="setting-row">
				...
				<view class="datetime-pick">
					<picker mode="date" :value="form.deadlineDate" @change="onDateChange">
						<text class="picker-value">{{ form.deadlineDate || t('vote.selectDate') }} ▾</text>
					</picker>
					<picker mode="time" :value="form.deadlineTime" @change="onTimeChange">
						<text class="picker-value">{{ form.deadlineTime || '23:59' }} ▾</text>
					</picker>
				</view>
			</view>
			<view class="setting-row">
				<text class="setting-label">{{ t('vote.anonymous') }}</text>
				<switch :checked="form.anonymous" @change="form.anonymous = $event.detail.value" color="#1677FF" />
			</view>
			<button class="btn-primary" @tap="editingId ? saveEdit() : submitVote()">
				{{ editingId ? t('vote.saveEdit') : t('vote.submit') }}
			</button>
			<text v-if="editingId" class="cancel-edit" @tap="cancelEdit">{{ t('vote.cancelEdit') }}</text>
		</view>
		...
		<scroll-view scroll-y refresher-enabled :refresher-triggered="refreshing" @refresherrefresh="onRefresh" style="height: calc(100vh - 400rpx);">
			...
			<view v-for="(vote, index) in voteList" :key="vote._id || index"
				:class="['card', 'vote-item', vote.anonymous ? 'anonymous-card' : '']" @tap="showDetail(vote)">
				<view class="vote-header">
					<text class="vote-title">{{ vote.title }}</text>
					<view class="vote-tags">
						<text v-if="vote.anonymous" class="vote-tag anonymous">🔒 {{ t('vote.anonymousTag') }}</text>
				...
				<view v-if="vote.deadline" class="vote-deadline">
					<text>{{ t('vote.deadline') }}：{{ formatDateTime(vote.deadline) }}</text>
				</view>
				<view v-for="(opt, oi) in vote.options" :key="oi" class="vote-option" @tap.stop="doVoteAndRefresh(vote, oi)">
					<view v-if="hasCurrentUserVoted(vote)" class="option-bar">
						<view class="option-fill" :style="{ width: getPercent(vote, oi) + '%' }"></view>
					</view>
					<view v-if="hasCurrentUserVoted(vote)" class="option-info">
						<text>{{ opt.label }}</text>
				...
				<view class="vote-footer">
					...
					<view class="vote-actions-row">
						<text class="action-link edit" @tap.stop="startEdit(vote)">{{ t('vote.edit') }}</text>
						<text class="action-link delete" @tap.stop="deleteVote(vote)">{{ t('vote.deleteBtn') }}</text>
					</view>
				</view>
			</view>
		</scroll-view>
			...
			<view class="modal-content" @tap.stop>
				...
				<view class="detail-section">
					...
					<view class="detail-row">
						...
						<text class="detail-value">{{ detailItem.creatorId?.nickname || '-' }}</text>
					</view>
					<view class="detail-row">
						<text class="detail-label">{{ t('vote.deadline') }}</text>
						<text class="detail-value">{{ formatDateTime(detailItem.deadline) }}</text>
					</view>
					<view v-if="hasCurrentUserVoted(detailItem)" class="detail-row">
						<text class="detail-label">{{ t('vote.totalVotes') }}</text>
						<text class="detail-value">{{ getTotalVotes(detailItem) }} {{ t('vote.ticket') }}</text>
					</view>
					<view class="detail-row">
						<text class="detail-label">{{ t('vote.voteType') }}</text>
						<text :class="['detail-value', detailItem.anonymous ? 'anonymous-yes' : '']">
							{{ detailItem.anonymous ? '🔒 ' + t('vote.anonymousTag') : t('vote.publicVote') }}
						</text>
					</view>
				</view>
				<view class="detail-divider"></view>
				<view class="detail-options">
					<view v-for="(opt, oi) in detailItem.options" :key="oi" class="detail-opt">
						<view class="detail-opt-header">
							<text class="detail-opt-label">{{ opt.label }}</text>
							<text v-if="hasCurrentUserVoted(detailItem)" class="detail-opt-count">{{ opt.count }} {{ t('vote.ticket') }} ({{ getPercent(detailItem, oi) }}%)</text>
						</view>
						<view v-if="hasCurrentUserVoted(detailItem)" class="detail-opt-bar">
							<view class="detail-opt-fill" :style="{ width: getPercent(detailItem, oi) + '%' }"></view>
						</view>
						<view v-if="!detailItem.anonymous && hasCurrentUserVoted(detailItem) && getVoterNames(detailItem, oi).length > 0" class="detail-voters">
							<text class="voter-text">👤 {{ getVoterNames(detailItem, oi).join('、') }}</text>
						</view>
					</view>
				</view>
				<view v-if="detailItem.status === 'active' && !hasCurrentUserVoted(detailItem)" class="detail-actions">
					<text class="detail-tip">{{ t('vote.tapToVote') }}</text>
					<view class="detail-vote-btns">
						<view v-for="(opt, oi) in detailItem.options" :key="oi" class="detail-vote-btn" @tap="doVoteFromDetail(detailItem, oi)">
							<text>{{ opt.label }}</text>
						</view>
					</view>
				</view>
...
const editingId = ref('')
const form = reactive({ title: '', options: ['', ''], deadlineDate: '', deadlineTime: '23:59', anonymous: false })
const voteList = ref([])
...
const onDateChange = (e) => { form.deadlineDate = e.detail.value }
const onTimeChange = (e) => { form.deadlineTime = e.detail.value }
...
const showDetail = (vote) => { detailItem.value = vote; detailVisible.value = true }

const doVote = async (vote, optIndex) => {
	if (vote.status !== 'active') { uni.showToast({ title: t('vote.voteEnded'), icon: 'none' }); return }
	try {
		const res = await post('/api/decision/vote', { decisionId: vote._id, optionIndex: optIndex })
		if (res.code === 200) { uni.showToast({ title: t('vote.voteSuccess'), icon: 'success' }) }
		else { uni.showToast({ title: res.msg || t('vote.voteEnded'), icon: 'none' }) }
	} catch (e) { uni.showToast({ title: t('vote.voteEnded'), icon: 'none' }) }
}
const doVoteFromDetail = async (vote, oi) => {
	await doVote(vote, oi); await fetchList()
	const updated = voteList.value.find(v => v._id === vote._id)
	if (updated) detailItem.value = updated
}
const doVoteAndRefresh = async (vote, oi) => { await doVote(vote, oi); await fetchList() }
...
const submitVote = async () => {
	...
	if (!form.deadlineDate) { uni.showToast({ title: t('vote.selectDeadline'), icon: 'none' }); return }
	const deadline = `${form.deadlineDate}T${form.deadlineTime || '23:59'}:00`
	const userInfo = getLocalUserInfo()
	try {
		const res = await post('/api/decision/create', {
			dormId: userInfo.dormId, title: form.title, options: validOptions,
			deadline, creatorId: userInfo._id, anonymous: form.anonymous
		})
		if (res.code === 200) {
			uni.showToast({ title: t('vote.created'), icon: 'success' })
			resetForm(); showForm.value = false; fetchList()
...
const cancelEdit = () => { resetForm(); editingId.value = ''; showForm.value = false }
const saveEdit = async () => {
	...
	try {
		const res = await put('/api/decision/update', {
			decisionId: editingId.value, title: form.title, deadline, anonymous: form.anonymous
		})
		if (res.code === 200) {
			uni.showToast({ title: t('vote.editSuccess'), icon: 'success' })
			resetForm(); editingId.value = ''; showForm.value = false; fetchList()
		} else { uni.showToast({ title: res.msg, icon: 'none' }) }
	} catch (e) { console.error(e) }
}
...
const deleteVote = (vote) => {
	uni.showModal({
		title: t('vote.deleteTitle'), content: t('vote.deleteConfirm'),
		success: async (res) => {
			if (!res.confirm) return
			try {
				const r = await del('/api/decision/delete', { decisionId: vote._id })
				if (r.code === 200) { uni.showToast({ title: t('vote.deleted'), icon: 'success' }); fetchList() }
			} catch (e) { console.error(e) }
		}
	})
}
...
const resetForm = () => {
	form.title = ''; form.options = ['', '']; form.deadlineDate = ''; form.deadlineTime = '23:59'; form.anonymous = false
}
...
.datetime-pick { display: flex; gap: 16rpx; }
.picker-value { font-size: 28rpx; color: var(--color-primary); }
.cancel-edit { display: block; text-align: center; color: var(--text-hint); font-size: 26rpx; margin-top: 16rpx; }
...
/* 投票卡片 */
.vote-item { margin-bottom: 16rpx; }
.anonymous-card { border-left: 6rpx solid #ff9800; }
.vote-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16rpx; }
.vote-title { font-size: 30rpx; font-weight: bold; color: var(--text-primary); flex: 1; }
.vote-tags { display: flex; gap: 8rpx; align-items: center; flex-shrink: 0; }
.vote-tag { font-size: 22rpx; padding: 2rpx 12rpx; border-radius: 16rpx; }
.vote-tag.anonymous { background-color: #fff3e0; color: #e65100; }
.dark-mode .vote-tag.anonymous { background-color: #3e2700; color: #ffb74d; }
...
.option-pending {
	...
	background: var(--bg-input); border-radius: 8rpx; font-size: 26rpx;
	color: var(--text-primary);
}
.option-pending:active { background: var(--color-primary); color: #fff; opacity: 0.8; }
.vote-footer { display: flex; justify-content: space-between; align-items: center; margin-top: 12rpx; }
.vote-time { font-size: 22rpx; color: var(--text-placeholder); }
.vote-actions-row { display: flex; gap: 24rpx; }
.action-link { font-size: 24rpx; padding: 4rpx 8rpx; }
...
.detail-section { margin-bottom: 8rpx; }
...
.detail-value { font-size: 26rpx; color: var(--text-primary); font-weight: 500; }
.anonymous-yes { color: #e65100; }
.detail-divider { height: 1rpx; background: var(--border-color); margin: 20rpx 0; }
.detail-options { margin-bottom: 20rpx; }
.detail-opt { margin-bottom: 20rpx; }
.detail-opt-header { display: flex; justify-content: space-between; margin-bottom: 8rpx; }
.detail-opt-label { font-size: 28rpx; color: var(--text-primary); font-weight: 500; }
.detail-opt-count { font-size: 24rpx; color: var(--color-primary); }
.detail-opt-bar { height: 24rpx; background: var(--bg-input); border-radius: 12rpx; overflow: hidden; }
.detail-opt-fill { height: 100%; background: var(--color-primary); border-radius: 12rpx; opacity: 0.6; transition: width 0.3s; }
.detail-voters { margin-top: 8rpx; }
.voter-text { font-size: 22rpx; color: var(--text-hint); }
.detail-actions { margin-top: 16rpx; }
.detail-tip { font-size: 24rpx; color: var(--text-hint); text-align: center; display: block; margin-bottom: 16rpx; }
.detail-vote-btns { display: flex; flex-wrap: wrap; gap: 16rpx; }
.detail-vote-btn { flex: 1; min-width: 40%; text-align: center; padding: 16rpx 0; background: var(--color-primary); color: #fff; border-radius: 12rpx; font-size: 26rpx; }
.detail-vote-btn:active { opacity: 0.8; }
.voted-tip { text-align: center; padding: 24rpx 0; color: #4caf50; font-size: 28rpx; }
</style>
