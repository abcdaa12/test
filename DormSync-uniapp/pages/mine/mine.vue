  ...
  <view :class="['page', isDark ? 'dark-mode' : '']">
    ...
    <view class="user-header">
      ...
      <view class="user-info">
        <text class="user-name">{{
          userInfo.nickname || t("mine.defaultSign")
        }}</text>
        <text class="user-sign">{{
          userInfo.signature || t("mine.defaultSign")
        }}</text>
      </view>
    </view>
    ...
    <view class="container">
      <view v-if="!loggedIn" class="login-card">
        ...
        <button class="btn-login" @tap="handleLogin">
          {{ t("mine.wxLogin") }}
        </button>
      </view>
...
const toggleDarkMode = () => {
  toggleDark();
  uni.showToast({
    title: isDark.value ? t("mine.darkOn") : t("mine.darkOff"),
    icon: "none",
  });
};
...
const logout = () => {
  uni.showModal({
    ...
    success: (res) => {
      if (res.confirm) {
        clearAuth();
        closeWebSocket(); // 退出登录时断开 WebSocket
